...
void proto_register_tusovit(void);
static int hf_tusovit_info_type = -1;
...
/* Alexey */
static int proto_tusovit = -1;
static gint ett_tusovit = -1;
...
dissect_infiniband_common(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, ib_packet_start_header starts_with)
{
    ...
    if (starts_with == IB_PACKET_STARTS_WITH_GRH) {
        ...
        goto skip_lrh;
         ...
         col_set_str(pinfo->cinfo, COL_PROTOCOL, "TUSOVit");
         lnh_val = IBA_LOCAL;
         packetLength = tvb_reported_length_remaining(tvb, offset);
         crclen = 4;
         goto skip_lrh;
...
static void dissect_tusovit(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree){
	/* Top Level Item */
    proto_item *tusovit_packet;
	/* The Headers Subtree */
    proto_tree *all_headers_tree;
	gint offset = 0;                /* Current Offset */
	
	tusovit_packet = proto_tree_add_item(tree, proto_tusovit, tvb, offset, -1, ENC_NA);
	/* Headers Level Tree */
    all_headers_tree = proto_item_add_subtree(tusovit_packet, ett_tusovit);
	proto_tree_add_item(all_headers_tree, hf_tusovit_info_type, tvb, offset, 7, ENC_BIG_ENDIAN);
    ...
    else /* Normal Data Packet - Parse as such */
    {
        ...
        * We also need  to reserve 6 bytes for the CRCs which are not actually part of the payload.  */
        ...
        if (dissector_found == FALSE)
        {
			...
			/* Alexey - Here add our addional unwrap */
			
			
			//dissect_tusovit(next_tvb, pinfo, top_tree);
...
/* Alexey */
void proto_register_tusovit(void){
	static hf_register_info hf_tusovit[] = {
		{
			&hf_tusovit_info_type,
			{ "TUSOVit info", "tusovit.type",
			FT_BYTES, BASE_NONE, NULL, 0x0, NULL, HFILL}
		}
	};
	...
	proto_tusovit = proto_register_protocol("TUSOVit", "Tusov", "tusov");
	
	proto_register_field_array(proto_tusovit, hf_tusovit, array_length(hf_tusovit));
    proto_register_subtree_array(ett_tus, array_length(ett_tus));
}
